...
func RenderGraphPage(c *render.ComponentContext, p models.Page, children func(c *render.ComponentContext) error) error {
	...
	c.WriteString(" class=\"dashboard-card graph-hero\"")
	c.WriteString(">")
	c.WriteString(`<div`)
	...
	c.WriteString(">")
	c.WriteString(`Live force layout`)
	c.WriteString(`</div>`)
	c.WriteString(`<h2>`)
	c.WriteString(`FOLLOWS · `)
	c.WriteString(html.EscapeString(fmt.Sprint(len(p.Dash.Users))))
	c.WriteString(` nodes · `)
	...
	c.WriteString(`</div>`)
	c.WriteString(`<div`)
	c.WriteString(" id=\"graph-canvas\"")
	c.WriteString(" class=\"graph-canvas\"")
	c.WriteString(" aria-label=\"3D FOLLOWS graph\"")
	c.WriteString(" data-graph=\"")
	c.WriteString(html.EscapeString(fmt.Sprint(p.Dash.GraphJSON)))
	c.WriteString("\"")
	c.WriteString(">")
	c.WriteString(`</div>`)
	c.WriteString(`<p`)
	...
	c.WriteString(">")
	c.WriteString(`Drag to orbit · scroll to zoom · hover a node for its connections.`)
	c.WriteString(`</p>`)
	c.WriteString(`<script`)
	c.WriteString(" defer=\"\"")
	c.WriteString(" src=\"/static/vendor/three.min.js?v=3\"")
	c.WriteString(">")
	c.WriteString(`</script>`)
	c.WriteString(`<script`)
	c.WriteString(" defer=\"\"")
	c.WriteString(" src=\"/static/graph3d.js?v=3\"")
	c.WriteString(">")
	c.WriteString(`</script>`)
	c.WriteString(`</section>`)
	c.WriteString(`<div`)
...
func RenderEdgeList(c *render.ComponentContext, props map[string]any, children func(c *render.ComponentContext) error) error {
	...
	for _, s := range props["suggestions"].([]models.Suggestion) {
		...
		c.WriteString(`<a`)
		c.WriteString(" href=\"")
		c.WriteString(html.EscapeString(fmt.Sprint("/profile/" + s.ID)))
		c.WriteString("\"")
		c.WriteString(">")
		c.WriteString(`&#64;`)
